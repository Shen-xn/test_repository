...
class CustomLinearFunction(Function):
    ...
    def backward(ctx, grad_output):  # 定义逆向传播过程
        ...
        if ctx.use_grad_norm:
            # 计算学习率归一化系数
            alphas = (1 / (norms_O_prev + 1e-10)).view(-1, 1)       # (batch_size, 1)
            grad_output /= alphas

        # 应用梯度补偿
        if ctx.use_combination:
            if O_prev_prev_with_bias is not None and f_prime is not None and delta_k_minus1 is not None:
                # 计算范数比
                norms_O_prev_prev = torch.norm(O_prev_prev_with_bias, dim=1, keepdim=True).square()
                ratio = (norms_O_prev_prev / (norms_O_prev + 1e-10))
                ...
                # 应用补偿
                grad_output = grad_output - compensation
            # 如果要求使用补偿却没有给O_prev_prev_with_bias就会报错
            else:
                raise Exception(f'Wrrong situation when use_combination = {ctx.use_combination} but 
                                O_prev_prev_with_bias = {O_prev_prev_with_bias}, f_prime = {f_prime} and delta_k_minus1 = {delta_k_minus1} ')
...
class CustomLinear(nn.Module):
    def __init__(self, input_dim, output_dim, bias=True,
                 use_grad_norm=False, use_combination=False, gamma=0.1):
        super().__init__()
        self.weight = nn.Parameter(torch.Tensor(output_dim, input_dim + int(bias)))
        self.bias = bias
        self.use_grad_norm = use_grad_norm
        self.use_combination = use_combination
        self.gamma = gamma
    ...
    def forward(self, input, prev_layer_info=(None, None, None)):
        return CustomLinearFunction.apply(
            input, self.weight, self.bias,
            prev_layer_info,
            self.use_grad_norm, self.use_combination, self.gamma
        )
"""
