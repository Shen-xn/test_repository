...
class CustomLinearFunction(Function):
    ...
    def backward(ctx, grad_output):  # 定义逆向传播过程
        ...
        # 计算处理补偿所需的可复用材料
        if A_prev is not None and activation_fn is not None and O_prev_prev_with_bias is not None:
            # 计算激活函数导数
        ...
        if ctx.use_grad_norm:
            # 计算学习率归一化系数
            alphas = (1 / (norms_O_current + 1e-10)).view(-1, 1)       # (batch_size, 1)
            grad_output /= alphas

        # 梯度补偿计算
        if ctx.use_combination and O_prev_prev_with_bias is not None:
            # 计算范数比
            norms_O_prev_prev = torch.norm(O_prev_prev_with_bias, dim=1, keepdim=True).square()
            norms_O_current = torch.norm(O_k_minus1_with_bias, dim=1, keepdim=True).square()
            ratio = (norms_O_prev_prev / (norms_O_current + 1e-10))
