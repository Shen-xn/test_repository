...
class CustomLinearFunction(Function):
    @staticmethod
    def forward(ctx, input_with_bias, weight, bias):
    ...
    def backward(ctx, grad_output):
        ...
        # 调整梯度：逐样本乘以 alpha
        adjusted_grad_output = grad_output * alphas.view(-1, 1)  # (batch_size, output_dim) = (batch_size, output_dim)*(batch_size, 1)
        ...
        # 计算输入梯度
        grad_input = torch.mm(grad_output, weight)  # (batch_size, input_dim + 1)
...
class CustomLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算输出: (batch_size, output_dim) = (batch_size, input_dim + 1)*(output_dim, input_dim + 1).T
