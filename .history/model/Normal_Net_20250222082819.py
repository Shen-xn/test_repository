...
class CustomHardtanhFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, beta=0.1):
        # ctx.save_for_backward(x)
        ctx.output = torch.clamp(x, min=0)
        ctx.beta = beta  # 保存超参数 β
        return ctx.output  # 前向传播使用 Hardtanh
    ...
    def backward(ctx, grad_output):
        ...
        # grad_input[x > -1 and x < 1] = 1

        # 检查是否需要衰减梯度
        output = ctx.output
        mask = (output == 0) & (torch.sign(grad_output) == 1)  # 判断是否满足条件
        grad_input[mask] *= 0.0  # 衰减梯度
...
class NormNet(nn.Module):
    ...
    def reset_parameters(self):
        ...
        nn.init.uniform_(self.fc4.weight, a=-1.0, b=1.0)
