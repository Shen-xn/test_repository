...
class CustomLinearFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        # 计算输入梯度
        grad_input = torch.mm(adjusted_grad_output, weight)  # (batch_size, input_dim + 1)

        # 去掉偏置项的梯度（因为输入不需要偏置项的梯度）
        grad_input = grad_input[:, 1:]  # (batch_size, input_dim)
...
class CustomLinear(nn.Module):
    def __init__(self, input_dim, output_dim, beta=0.01):
        super().__init__()
        self.weight = nn.Parameter(torch.Tensor(output_dim, input_dim + 1))  # 合并偏置项
        self.beta = beta
        nn.init.xavier_uniform_(self.weight)

    def forward(self, x):
        return CustomLinearFunction.apply(x, self.weight, self.beta)
