...
class NormNet(nn.Module):
    ...
    def custom_backward(self, loss):
        ...
        layer_input_names = ["x", "o1", "o2"]  # 每层的输入名称

        for layer, input_name in zip(layers, layer_input_names):
          
            # 找到相应的输入
            O_prev = self.history[-1][input_name]  # 获取上一层的输出（包含偏置项）
            O_prev = torch.cat([torch.ones(O_prev.size(0), layer.bias, device=O_prev.device), O_prev], dim=1)

            # 计算 L2 范数
            norm_O_prev = torch.norm(O_prev, p=2)

            # 计算新的学习率
            alpha = self.beta / (norm_O_prev ** 2)
...
def compute_weighted_loss(model, outputs, targets, beta=0.01, layer_name="fc1"):
    """
    根据指定层的输入计算加权损失
    ...
    # 获取该层的输入（含偏置项）
    if layer_name == "fc1":
        O_prev = model.fc1_input  # 形状: (batch_size, input_size+1)
    elif layer_name == "fc2":
        O_prev = model.fc2_input  # 形状: (batch_size, hidden_size+1)
    elif layer_name == "fc3":
        O_prev = model.fc3_input  # 形状: (batch_size, hidden_size+1)
    else:
