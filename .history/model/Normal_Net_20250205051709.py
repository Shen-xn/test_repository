...
class NormNet(nn.Module):
    ...
    def input_history_norms(self):
        ...
        norms = {"x":torch.tensor(1),"o1": torch.tensor(1),"o2": torch.tensor(1)}
        ...
        if len(self.history) == 0:
            ...
            for layer, input_name in zip(layers, layer_input_names):
                ...
                O_prev_with_bias = torch.cat([torch.ones(O_prev.size(0),layer.bias, device=O_prev.device), O_prev], dim=1)
                
                # 计算 L2 范数的平方（整个 batch 的模）
                norm_O_prev_sq = torch.sum(O_prev_with_bias ** 2, dim=1)
