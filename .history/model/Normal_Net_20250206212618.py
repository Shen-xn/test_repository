...
"""我们将来所有的补偿都会在这里加入！"""
class CustomLinearFunction(Function):  # 自定义的层误差正向和逆向传播过程
    @staticmethod
    def forward(ctx, input, weight, bias, use_grad_norm):  # 定义这一块的正向传播过程
        ...
        # 为输入加上偏置
        ctx.bias = int(bias)
        input_with_bias = torch.cat([torch.ones(input.size(0), ctx.bias, device=input.device), input], dim=1)
    ...
    def backward(ctx, grad_output):  # 定义逆向传播过程
        ...
        input_with_bias, weight = ctx.saved_tensors  # (batch_size, output_dim), (input_dim + 1, output_dim)

        if ctx.use_grad_norm:
        # 计算每个样本的 L2 范数平方
            norms_sq = torch.sum(input_with_bias ** 2, dim=1)  # (batch_size,)
            # 计算学习率修正系数
            alphas = (1 / (norms_sq + 1e-9)).view(-1, 1)       # (batch_size, 1)
        else:
            # 如果不适用梯度归一化就只让 alphas=1 就行
            alphas = torch.ones_like(grad_output)

        # 调整梯度：逐样本乘以 alpha - 这里我们直接乘在了delta上
        adjusted_grad_output = grad_output * alphas  # (batch_size, output_dim) = (batch_size, output_dim)*(batch_size, 1) or (batch_size, output_dim)*(batch_size, output_dim)
...
class CustomLinear(nn.Module):
    def __init__(self, input_dim, output_dim, bias=1, use_grad_norm=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return CustomLinearFunction.apply(x, self.weight, self.bias, self.use_grad_norm)


# 模型基本结构
class NormNet(nn.Module):
    # 初始化神经网络类, 需要参数(激活函数，隐藏层维度，是否使用batchnorm)
    def __init__(self, activation_func, layer_dim, usebatchnorm=False, use_grad_norm=False):  
        ...
        self.fc1 = CustomLinear(1, self.__dim, bias=1, use_grad_norm=use_grad_norm)  # 输入层到隐层的线性变换 (batch_size, 1)->(batch_size, dim)
        self.fc2 = CustomLinear(self.__dim, self.__dim, bias=1, use_grad_norm=use_grad_norm) 
        self.fc3 = CustomLinear(self.__dim, self.__dim, bias=1, use_grad_norm=use_grad_norm)
        self.output_layer = CustomLinear(self.__dim, 1, bias=0, use_grad_norm=use_grad_norm)  # (batch_size, dim)->(batch_size, 1)

        self.batchnorm1 = nn.BatchNorm1d(
            num_features=self.__dim,
            eps=1e-09,  # 为避免除以零的一个小值
            momentum=0.8,  # 运行时统计量的动量
    ...
    def single_train_iter(self, data_set, batch_range, criterion, optimizers):
...
def train_normalnet(model, data_set, val_set, criterion, optimizers, scheduler=None, batch_size=100, num_epochs=1000, show_epoch=20, print_info=True):
    ...
    for epoch in range(num_epochs):
        ...
        running_loss = model.single_train_iter(data_set, batch_range, criterion, optimizers)
