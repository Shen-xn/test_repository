...
class CustomLinearFunction(Function):  # 自定义的层误差正向和逆向传播过程
    @staticmethod
    def forward(ctx, input_with_bias, weight, bias, use_grad_norm):  # 定义这一块的正向传播过程
...
class CustomLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算输出: (batch_size, output_dim) = (batch_size, input_dim + 1)*(output_dim, input_dim + 1).T, 同时定义这个过程的逆传播过程，见CustomLinearFunction类，输入向量x中的1我们会在function内部添加
