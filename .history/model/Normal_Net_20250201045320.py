...
def show_element_activation(model, val_set):
...
def validation(model, val_set, criterion, take_history=False):
    model.eval()
    ...
    if take_history:
        show_element_activation(model, val_set)

    return loss

def train_normalnet(model, data_set, val_set, criterion, optimizer, num_epochs=10, batch_size=10, batch_in_epoch=10, show_epoch=100):
      # 设置模型为训练模式
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(batch_in_epoch):
            batch_range = torch.arange(i*batch_size, (i+1)*batch_size) % num_datas
        ...
        # 计算每个epoch的平均损失
        epoch_loss = running_loss / batch_in_epoch
        losses.append(epoch_loss.numpy())
        
        if (epoch + 1) % show_epoch == 0:
            val_loss = validation(model, val_set, criterion).detach()
