...
class CustomHardtanh(nn.Module):
    def __init__(self, beta=0.1):
        ...
        self.beta = beta  # 超参数 β

    def forward(self, x):
        # 前向传播使用标准的 Hardtanh 激活函数
        return F.hardtanh(x)

    def backward(self, grad_output):
        # 获取前向传播的输入和输出
        input = self.saved_input
        ...
        grad_input[mask] *= self.beta  # 衰减梯度
...
class CustomHardtanhFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, beta):
        ...
        return Function.hardtanh(x)  # 前向传播使用 Hardtanh
