...
class NormNet(nn.Module):
    ...
    def __init__(self, activation_func, layer_dim, usebatchnorm=False, usenormgrad=False):  
        ...
        self.fc1 = CustomLinear(1, self.__dim, bias=1, usenormgrad=usenormgrad)  # 输入层到隐层的线性变换 (batch_size, 1)->(batch_size, dim)
        self.fc2 = CustomLinear(self.__dim, self.__dim, bias=1, usenormgrad=usenormgrad) 
        self.fc3 = CustomLinear(self.__dim, self.__dim, bias=1, usenormgrad=usenormgrad)
        self.output_layer = CustomLinear(self.__dim, 1, bias=0, usenormgrad=usenormgrad)  # (batch_size, dim)->(batch_size, 1)
