    ...
    def backward(ctx, grad_output):  # 定义逆向传播过程
        ...
        if ctx.use_combination:
            if O_prev_prev is not None and f_prime is not None and delta_k_minus1 is not None:
                # 计算范数比
                norms_O_prev_prev = torch.norm(O_prev_prev, dim=1, keepdim=True).square()
            ...
            # 如果要求使用补偿却没有给O_prev_prev_with_bias就会报错
                ...
                raise Exception(f'Wrrong situation when use_combination = {ctx.use_combination} but 
                                O_prev_prev_with_bias = {O_prev_prev}, f_prime = {f_prime} and delta_k_minus1 = {delta_k_minus1} ')
...
class NormNet(nn.Module):
    ...
    def forward(self, x):
        ...
        v = self.fc3(o2, prev_layer_info=(o1, a2, self.activation_func))
