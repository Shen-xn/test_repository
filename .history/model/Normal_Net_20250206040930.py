...
class CustomLinearFunction(Function):
    @staticmethod
    def forward(ctx, input_with_bias, weight, bias):  # 定义这一块的正向传播过程
    ...
    def backward(ctx, grad_output):  # 定义逆向传播过程
        ...
        # 计算学习率修正系数
        alphas = 1 / (norms_sq + 1e-9)       # (batch_size,)

        # 调整梯度：逐样本乘以 alpha - 这里我们直接乘在了delta上
        adjusted_grad_output = grad_output * alphas.view(-1, 1)  # (batch_size, output_dim) = (batch_size, output_dim)*(batch_size, 1)bc
        ...
        # 计算输入梯度
        grad_input = torch.mm(grad_output, weight.t())  # (batch_size, input_dim + 1) = (batch_size, output_dim)* (input_dim + 1, output_dim)

        # 去掉偏置项的梯度（因为输入不需要偏置项的梯度）
        grad_input = grad_input[:, ctx.bias:]  # (batch_size, input_dim)

        return grad_input, grad_weight, None  # 假设 forward 方法的输入是 input、weight 和 beta，那么 backward 方法的返回值应该与这些输入一一对应！
...
class CustomLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        return CustomLinearFunction.apply(x_with_bias, self.weight, self.bias)
