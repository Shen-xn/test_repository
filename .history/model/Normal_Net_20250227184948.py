...
class CustomLinearFunction(Function):
    ...
    def forward(ctx, 
               ...
               use_grad_norm=False, use_combination=False, gamma=None, take_rand=False, use_comb=0):  # 定义这一块的正向传播过程
        """
        ...
        ctx.gamma = gamma
        ctx.take_rand = take_rand
        ctx.use_comb = use_comb
    ...
    def backward(ctx, grad_output):  # 定义逆向传播过程
        ...
        if ctx.use_combination and ctx.use_comb==0: 
        # and torch.randint(0, 2, (1,)) < 1:
            if O_prev_prev is not None and f_prime is not None and delta_k_minus1 is not None:
                ...
                # 计算补偿项
                compensation = ratio * torch.mm(delta_O_temp, weight.t())

                if ctx.take_rand == True:

                    # 生成与 compensation 相同大小的均匀分布随机矩阵，范围在 [0, 1]
                    uniform_random_matrix = torch.rand(compensation.size())

                    # 将均匀分布缩放到 [-1.5, 1.5]
                    scaled_uniform_matrix = uniform_random_matrix * 3 - 1.5

                    compensation *= scaled_uniform_matrix

                # 应用补偿
                grad_output = (1 - ctx.gamma)*grad_output - ctx.gamma*compensation
        ...
        return (
            ...
            None, None, None, None, None  # 超参数不需要梯度
        )
    

# 自定义线性层
class CustomLinear(nn.Module):
    def __init__(self, input_dim, output_dim, bias=1, use_grad_norm=False, use_combination=False, gamma=1, take_rand=False):
        super().__init__()
        # 层超参数
    ...
    def forward(self, o_k, prev_layer_info=(None, None, None), use_comb=0):
        ...
        return CustomLinearFunction.apply(o_k, self.weight, self.bias, prev_layer_info, self.use_grad_norm, self.use_combination, self.gamma, self.take_rand, use_comb)


"""
定义CustomHardtanh激活函数并封装
"""
# 将自定义激活函数封装为 PyTorch 函数
# 将自定义激活函数封装为 PyTorch 函数
class CustomHardtanhFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, beta):
        ctx.save_for_backward(x)  # 保存输入以供反向传播使用
        ctx.beta = beta  # 保存超参数 β
        return torch.clamp(x, min=-1, max=1)  # 使用 clamp 实现 Hardtanh

    @staticmethod
    def backward(ctx, grad_output):
        x, = ctx.saved_tensors  # 获取保存的输入
        beta = ctx.beta  # 获取超参数 β

        # 默认梯度规则
        grad_input = grad_output.clone()

        # 检查是否需要衰减梯度
        output = torch.clamp(x, min=-1, max=1)  # 计算前向传播的输出
        mask = (output == - torch.sign(grad_output))  # 判断是否满足条件
        grad_input[mask] *= beta  # 衰减梯度

        return grad_input, None  # 返回输入的梯度和 None（因为 beta 不需要梯度）

# 封装为模块
class CustomHardtanhLayer(nn.Module):
    def __init__(self, beta=0.1):
        super(CustomHardtanhLayer, self).__init__()
        self.beta = beta

    def forward(self, x):
        return CustomHardtanhFunction.apply(x, self.beta)
...
class NormNet(nn.Module):
    ...
    def __init__(self, activation_func, layer_dim, usebatchnorm=False, use_grad_norm=False, use_combination=False, gamma=0, take_rand=False):  
        ...
        self.fc1 = CustomLinear(1, self.__dim, bias=1, use_grad_norm=use_grad_norm, use_combination=False, gamma=gamma, take_rand=take_rand)  # 输入层到隐层的线性变换 (batch_size, 1)->(batch_size, dim)
        self.fc2 = CustomLinear(self.__dim, self.__dim, bias=1, use_grad_norm=use_grad_norm, use_combination=use_combination, gamma=gamma, take_rand=take_rand) 
        self.fc3 = CustomLinear(self.__dim, self.__dim, bias=1, use_grad_norm=use_grad_norm, use_combination=use_combination, gamma=gamma, take_rand=take_rand)
        self.fc4 = CustomLinear(self.__dim, 1, bias=1, use_grad_norm=use_grad_norm, use_combination=use_combination, gamma=gamma, take_rand=take_rand)
    ...
    # 前向传播过程
    def forward(self, x):
        random_number = torch.randint(low=1, high=4, size=(1,)).item()
        if x.dim() != 2: 
        ...
        a2 = self.fc2(o1, prev_layer_info=(x, a1, self.activation_func), use_comb=random_number-1)
        o2 = self.activation_func(a2)
        if self.usebatchnorm == True:
            o2 = self.batchnorm2(o2)  # 应用 BatchNorm1d 层
        a3 = self.fc3(o2, prev_layer_info=(o1, a2, self.activation_func), use_comb=random_number-2)
        o3 = self.activation_func(a3)
        if self.usebatchnorm == True:
            o3 = self.batchnorm3(o3)  # 应用 BatchNorm1d 层
        a4 = self.fc4(o3, prev_layer_info=(o2, a3, self.activation_func), use_comb=random_number-3)
    ...
    def reset_parameters(self):
        # 手动设置fc1的权重和偏置
        if self.activation_func._get_name() == "CustomHardtanhLayer":
            std = 2
        else:
            std = 0.5
        # nn.init.normal_(self.fc1.weight, mean=0, std=std)  # 初始化
        # nn.init.normal_(self.fc2.weight, mean=0, std=std)
        # nn.init.normal_(self.fc3.weight, mean=0, std=std)
        # nn.init.normal_(self.fc4.weight, mean=0, std=std)
        # nn.init.normal_(self.output_layer.weight, mean=0, std=0.1)  # Xavier初始化

        nn.init.uniform_(self.fc1.weight, a=-std, b=std)
        nn.init.uniform_(self.fc2.weight, a=-std, b=std)
        nn.init.uniform_(self.fc3.weight, a=-std, b=std)
        nn.init.uniform_(self.fc4.weight, a=-std, b=std)
