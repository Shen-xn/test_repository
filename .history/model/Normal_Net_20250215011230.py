...
class CustomLinearFunction(Function):  # 自定义的层误差正向和逆向传播过程
    @staticmethod
    def forward(ctx, input, weight, bias, use_grad_norm=False, use_combination=False):  # 定义这一块的正向传播过程
    ...
    def backward(ctx, grad_output):  # 定义逆向传播过程
        ...
        # 我们要在delta上进行补偿，需要的材料：
            # 1. 对于线性部分: (delta_k-1, O_k-2) & A_k-1， 使用人为构造Function映射计算
            # 2. 对于非线性部分（或存在精度差的普适方法）：(delta_k-1, O_k-2) & A_k-1
...
class NormNet(nn.Module):
    ...
    def __init__(self, activation_func, layer_dim, usebatchnorm=False, use_grad_norm=False):  
        ...
        self.output_layer = CustomLinear(self.__dim, 1, bias=0, use_grad_norm=False)  # (batch_size, dim)->(batch_size, 1)
    ...
    def reset_parameters(self):
        # 手动设置fc1的权重和偏置
        nn.init.normal_(self.fc1.weight, mean=0, std=0.1)  # Xavier初始化
        nn.init.normal_(self.fc2.weight, mean=0, std=0.1)  # Xavier初始化
        nn.init.normal_(self.fc3.weight, mean=0, std=0.1)  # Xavier初始化
