...
class CustomLinearFunction(Function):  # 自定义的层误差正向和逆向传播过程
    @staticmethod
    def forward(ctx, input_with_bias, weight, bias, use_grad_norm=False):  # 定义这一块的正向传播过程
...
class CustomLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # 添加全 1 列（偏置项）=> (batch_size, input_dim + 1)
        ...
        return CustomLinearFunction.apply(x_with_bias, self.weight, self.bias, use_grad_norm=self.use_grad_norm)
